...
function modify_password_button(){
    ...
    if(password_again == '' || password == '' || new_password == ''){
        ...
        if(new_password != password_again) window.alert("请输入相同的密码");
            ...
            if (re.test(account_receive) || re.test(password) || re.test(new_password))  window.alert('输入信息中存在特殊字符！');
            else {
...
function delete_button(){
    ...
    //限制用户输入，防止sql注入攻击和XSS存储攻击
    var re=/select|update|delete|truncate|join|union|exec|insert|drop|count|'|"|;|>|<|%/i;
    if (re.test(name) || re.test(account_receive)) window.alert('输入信息中包含特殊字符！');
    else {
        xhttp.onreadystatechange = function () {
            if(this.readyState == 4 && this.status == 200) {
...
function selectFile(e) {
    ...
    if(e.target.files[0].type == 'application/pdf') {
        ...
        //限制用户输入，防止sql注入攻击和XSS存储攻击
        var re=/select|update|delete|truncate|join|union|exec|insert|drop|count|'|"|;|>|<|%/i;
        if (re.test(file['name']) || re.test(account_receive))  window.alert('文件名称中存在特殊字符！');
        else {
...
function Reading_analysis() {
    ...
    //限制用户输入，防止sql注入攻击和XSS存储攻击
    var re=/select|update|delete|truncate|join|union|exec|insert|drop|count|'|"|;|>|<|%/i;
    if (re.test(account_receive))  window.alert('输入信息存在特殊符号！');
    else {
        var xhttp = new XMLHttpRequest();
        xhttp.onreadystatechange = function () {
            if(this.readyState == 4 && this.status == 200) {
...
function send_message_button() {
    ...
    if(lang_input != '' && tag_input != ''){
        //限制用户输入，防止sql注入攻击和XSS存储攻击
        var re=/select|update|delete|truncate|join|union|exec|insert|drop|count|'|"|;|>|<|%/i;
        if (re.test(name_input) || re.test(tag_input) || re.test(lang_input) || re.test(account_receive) || re.test(data))
            window.alert('输入信息中含有特殊符号');
        else {
            var xhttp = new XMLHttpRequest();
            xhttp.onreadystatechange = function () {
                if(this.readyState == 4 && this.status == 200) {
...
function showResult(str){
    ...
    //限制用户输入，防止sql注入攻击和XSS存储攻击
    var re=/select|update|delete|truncate|join|union|exec|insert|drop|count|'|"|;|>|<|%/i;
    if (re.test(account) || re.test(input)) window.alert('输入内容中含有特殊符号！');
    else {
        var xhttp = new XMLHttpRequest();
        xhttp.onreadystatechange = function () {
            if(this.readyState == 4 && this.status == 200) {
                    ...
                    if(xmlDoc.getElementsByTagName("name").length == 0){
                        ...
                        for(i = 0; i < x; ++i) {
                            ...
                            document.getElementById(pdf_id).onclick = function () {
                                ...
                                //限制用户输入，防止sql注入攻击和XSS存储攻击
                                var re=/select|update|delete|truncate|join|union|exec|insert|drop|count|'|"|;|>|<|%/i;
                                if (re.test(path)) window.alert('文件路径含有特殊字符！');
                                else {
                                    var xhttp = new XMLHttpRequest();
                                    xhttp.onreadystatechange = function () {
                                        if(this.readyState == 4 && this.status == 200) {
                            ...
                            document.getElementById(button_id).onclick = function (){
                                ...
                                //限制用户输入，防止sql注入攻击和XSS存储攻击
                                var re=/select|update|delete|truncate|join|union|exec|insert|drop|count|'|"|;|>|<|%/i;
                                if (re.test(account) || re.test(path) || re.test(name))  window.alert('输入信息中含有特殊字符！');
                                else {
                                    var xhttp = new XMLHttpRequest();
                                    xhttp.onreadystatechange = function () {
                                        if(this.readyState == 4 && this.status == 200) {
                            ...
                            document.getElementById(button_id_copy).onclick = function () {
                                ...
                                //限制用户输入，防止sql注入攻击和XSS存储攻击
                                var re=/select|update|delete|truncate|join|union|exec|insert|drop|count|'|"|;|>|<|%/i;
                                if (re.test(account) || re.test(path) || re.test(data))  window.alert('输入信息中含有特殊字符！');
                                else {
                                    var xhttp = new XMLHttpRequest();
                                    xhttp.onreadystatechange = function () {
                                        if(this.readyState == 4 && this.status == 200) {
...
function flush_pdf_table() {
    ...
    //限制用户输入，防止sql注入攻击和XSS存储攻击
    var re=/select|update|delete|truncate|join|union|exec|insert|drop|count|'|"|;|>|<|%/i;
    if (re.test(account_receive))  window.alert('输入信息中含有特殊字符！');
    else {
        var xhttp = new XMLHttpRequest();
        var n = 0;
        xhttp.onreadystatechange = function () {
                ...
                if(xmlDoc.getElementsByTagName("name").length == 0){
                    ...
                    for(i = 0; i < x; ++i) {
                        ...
                        document.getElementById(button_id).onclick = function (){
                            ...
                            //限制用户输入，防止sql注入攻击和XSS存储攻击
                            var re=/select|update|delete|truncate|join|union|exec|insert|drop|count|'|"|;|>|<|%/i;
                            if (re.test(account) || re.test(path) || re.test(name))  window.alert('输入内容含有特殊字符！');
                            else {
                                var xhttp = new XMLHttpRequest();
                                xhttp.onreadystatechange = function () {
                                    if(this.readyState == 4 && this.status == 200) {
                        ...
                        document.getElementById(button_id_copy).onclick = function () {
                            ...
                            var re=/select|update|delete|truncate|join|union|exec|insert|drop|count|'|"|;|>|<|%/i;
                            var xhttp = new XMLHttpRequest();
                            if (re.test(account) || re.test(path) || re.test(data))  window.alert('输入内容含有特殊字符！');
                            else {
                                xhttp.onreadystatechange = function () {
                                    if(this.readyState == 4 && this.status == 200) {
                                        flush_pdf_table();
...
function flush_card_table() {
    ...
    //限制用户输入，防止sql注入攻击和XSS存储攻击
    var re=/select|update|delete|truncate|join|union|exec|insert|drop|count|'|"|;|>|<|%/i;
    if (re.test(account_receive))  window.alert('输入内容中存在特殊符号！');
    else {
        var xhttp = new XMLHttpRequest();
        var n = 0;
        xhttp.onreadystatechange = function () {
...
function showCard(str) {
    ...
    //限制用户输入，防止sql注入攻击和XSS存储攻击
    var re=/select|update|delete|truncate|join|union|exec|insert|drop|count|'|"|;|>|<|%/i;
    if (re.test(account) || re.test(input))  window.alert('输入内容中存在特殊符号！');
    else {
        var xhttp = new XMLHttpRequest();
        xhttp.onreadystatechange = function () {
            if(this.readyState == 4 && this.status == 200) {
